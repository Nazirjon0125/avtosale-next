...
const TrendCarCard = (props: TrendCarCardProps) => {
	...
	const pushDetailHandler = async (carId: string) => {
		...
		await router.push({ pathname: '/property/detail', query: { id: carId } });
	};
			...
			<Card className="trend-car-card" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
				...
				<Box className="car-image-section">
					<Box
						className="car-image"
						style={{
							backgroundImage: `url(${REACT_APP_API_URL}/${car?.propertyImages[0]})`,
						}}
						onClick={() => pushDetailHandler(car._id)}
					/>

					{/* Price Badge */}
					<Box className="price-badge">${car.propertyPrice}</Box>
				...
				<CardContent className="car-content">
					...
					<Typography variant="body2" className="car-description">
						{car.propertyDesc || '4.0 D5 PowerPulse Momentum 5dr AW'}
					</Typography>
					...
					<Box className="car-footer">
						...
						<Box className="status-section">
							<Typography variant="body2" className={`status-text ${car.propertyBarter ? 'active' : 'inactive'}`}>
								Barter
							</Typography>
							<Typography variant="body2" className="status-divider">
								/
							</Typography>
							<Typography variant="body2" className={`status-text ${car.propertyRent ? 'active' : 'inactive'}`}>
								Rent
							</Typography>
						</Box>
